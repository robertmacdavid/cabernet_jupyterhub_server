...
#c.JupyterHub.data_files_path = '/u/macdavid/.local/share/jupyterhub'
...
c.JupyterHub.ssl_cert = '/srv/jupyterhub/jupyterhub-selfsigned.crt'

## Path to SSL key file for the public facing interface of the proxy
#  
#  When setting this, you should also set ssl_cert
c.JupyterHub.ssl_key = '/srv/jupyterhub/jupyterhub-selfsigned.key'
...
def create_dir_hook(spawner):
    ...
    notebook_dir = "/u/%s/%s/" % (username, start_dir)
    if not os.path.exists(notebook_dir):
        # create a directory with umask 0755 
        # hub and container user must have the same UID to be writeable
        # still readable by other users on the system
        uid = pwd.getpwnam(username).pw_uid
        gid = grp.getgrnam("grad").gr_gid
        os.mkdir(notebook_dir, 0o755)
...
#c.Spawner.notebook_dir = '~/%s/' % start_dir
c.Spawner.notebook_dir = "~"
